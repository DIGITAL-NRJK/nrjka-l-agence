...
export async function Footer({ locale = 'fr' }: { locale?: string }) {
  ...
  // Champs éditables ajoutés au global (types pas encore régénérés → accès souple).
  type FooterExtra = {
    tagline?: string | null
  ...
  const tagline =
    extra.tagline ||
    (en
      ? 'Your strategic digital partner to build sustainable growth.'
      : 'Votre partenaire digital et stratégique pour construire une croissance durable.')
  const description =
    extra.description ||
    (en
  ...
  const socialLinks = social
    ? (Object.keys(SOCIAL_PATHS)
        .map((key) => {
          const url = social[key as keyof typeof social]
          return url ? { key, url, ...SOCIAL_PATHS[key] } : null
        })
        .filter(Boolean) as { key: string; url: string; label: string; path: string }[])
    : []

  const contact = settings?.contact
  const addressParts = [contact?.postalCode, contact?.city].filter(Boolean).join(' ')
  const fullAddress = [contact?.addressLine, addressParts, contact?.country]
    .filter(Boolean)
    .join(', ')
  const hasContact = Boolean(contact?.email || contact?.phone || fullAddress)
      ...
      <div className="container py-14">
        <div className="flex flex-col gap-10 md:flex-row md:justify-between">
          <div className="max-w-xs">
            ...
            <p className="mt-4 text-base font-medium leading-relaxed text-white/90">{tagline}</p>
            ...
            {legalLinks.length > 0 ? (
              ...
              <>
                ...
                <Link
                  ...
                  className="transition-colors hover:text-white/80"
                >
                  {en ? 'Privacy policy' : 'Confidentialité'}
                </Link>
              </>
            )}
